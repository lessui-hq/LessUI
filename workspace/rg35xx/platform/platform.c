...
// For scaler-based LINE/GRID effects
...
void PLAT_setEffect(int effect) {
	...
	// LINE/GRID effects use scaler-based approach
...
scaler_t PLAT_getScaler(GFX_Renderer* renderer) {
	...
	// GRID effect - use scaler functions
	if (effect_type == EFFECT_GRID) {
		switch (renderer->scale) {
		case 3:
			return scale3x_grid;
		case 2:
...
void PLAT_flip(SDL_Surface* IGNORED, int sync) {
	...
	// write directly to the display engine's memory (DE2/Actions) rather than
	...
	vid.de_mem[DE_OVL_BA0(0) / 4] = vid.de_mem[DE_OVL_BA0(2) / 4] =
	    (uintptr_t)(vid.fb_info.padd + display_page * VIDEO_BUFFER_SIZE);
...
void PLAT_setCPUSpeed(int speed) {
	...
	switch (speed) {
	case CPU_SPEED_IDLE:
		freq = 408000; // 20% of max (298 → 408 MHz)
		break;
	case CPU_SPEED_POWERSAVE:
		freq = 816000; // 55% of max (819 → 816 MHz)
		break;
	case CPU_SPEED_NORMAL:
		freq = 1200000; // 80% of max (1190 → 1200 MHz)
		break;
	case CPU_SPEED_PERFORMANCE:
		freq = 1488000; // 100% (1488 MHz)
		break;
	}
