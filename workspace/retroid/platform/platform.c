...
// Treat Home, L3, R3 as menu buttons (L3/R3 repurposed - no analog stick click support)
 ...
 * LEDs are exposed as /sys/devices/platform/multi-led{l,r}{1-4}/leds/rgb:{l,r}{1-4}/brightness
 */
static void disableStickLEDs(void) {
	// LED device paths for left and right sticks (4 LEDs per stick)
	const char* led_paths[] = {
	    // Left stick LEDs
	    "/sys/devices/platform/multi-ledl1/leds/rgb:l1/brightness",
	    "/sys/devices/platform/multi-ledl2/leds/rgb:l2/brightness",
	    "/sys/devices/platform/multi-ledl3/leds/rgb:l3/brightness",
	    "/sys/devices/platform/multi-ledl4/leds/rgb:l4/brightness",
	    // Right stick LEDs
	    "/sys/devices/platform/multi-ledr1/leds/rgb:r1/brightness",
	    "/sys/devices/platform/multi-ledr2/leds/rgb:r2/brightness",
	    "/sys/devices/platform/multi-ledr3/leds/rgb:r3/brightness",
	    "/sys/devices/platform/multi-ledr4/leds/rgb:r4/brightness",
	};

	for (int i = 0; i < (int)(sizeof(led_paths) / sizeof(led_paths[0])); i++) {
		...
		while (read(input, &event, sizeof(event)) == sizeof(event)) {
			...
			if (type == EV_KEY) {
				...
				if (code == RAW_UP) {
					...
					btn = BTN_B; // Nintendo: bottom is B (RAW_A = BTN_SOUTH)
				} else if (code == RAW_B) {
					btn = BTN_A; // Nintendo: right is A (RAW_B = BTN_EAST)
				} else if (code == RAW_X) {
					btn = BTN_Y; // Nintendo: left is Y (RAW_X = BTN_WEST)
				} else if (code == RAW_Y) {
					btn = BTN_X; // Nintendo: top is X (RAW_Y = BTN_NORTH)
				...
				// Clamp to prevent overflow in scaling calculation
				if (code == RAW_LSX) {
					int clamped = value > 1408 ? 1408 : (value < -1408 ? -1408 : value);
					pad.laxis.x = (clamped * 32767) / 1408;
					PAD_setAnalog(BTN_ID_ANALOG_LEFT, BTN_ID_ANALOG_RIGHT, pad.laxis.x,
					              tick + PAD_REPEAT_DELAY);
				} else if (code == RAW_LSY) {
					int clamped = value > 1408 ? 1408 : (value < -1408 ? -1408 : value);
					pad.laxis.y = (clamped * 32767) / 1408;
					PAD_setAnalog(BTN_ID_ANALOG_UP, BTN_ID_ANALOG_DOWN, pad.laxis.y,
					              tick + PAD_REPEAT_DELAY);
				} else if (code == RAW_RSX) {
					int clamped = value > 1408 ? 1408 : (value < -1408 ? -1408 : value);
					pad.raxis.x = (clamped * 32767) / 1408;
				} else if (code == RAW_RSY) {
					int clamped = value > 1408 ? 1408 : (value < -1408 ? -1408 : value);
					pad.raxis.y = (clamped * 32767) / 1408;
				} else if (code == RAW_L2) {
...
// Minimum time for DSI panel brightness register to take effect before blanking.
// Empirically determined for AMOLED panels; may need adjustment for different panel revisions.
#define BRIGHTNESS_SETTLE_US 20000

void PLAT_enableBacklight(int enable) {
	if (enable) {
		// Restore brightness first, then unblank
		SetBrightness(GetBrightness());
		putInt(BLANK_PATH, FB_BLANK_UNBLANK);
	} else {
		// Set brightness to 0 before blanking to ensure screen turns off
		SetRawBrightness(0);
		usleep(BRIGHTNESS_SETTLE_US);
