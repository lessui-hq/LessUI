/**
 * minarch_video_convert.c - Pixel format conversion for MinArch
 *
...
int MinArchVideoConvert_needsConversion(MinArchPixelFormat format) {
	return format != MINARCH_PIXEL_FORMAT_RGB565;
}
...
void MinArchVideoConvert_convert(const void* data, unsigned width, unsigned height, size_t pitch,
	...
	size_t bytes_per_pixel = (format == MINARCH_PIXEL_FORMAT_XRGB8888) ? 4 : 2;
	...
	case MINARCH_PIXEL_FORMAT_RGB565:
		LOG_warn("MinArchVideoConvert_convert called for RGB565 (no conversion needed)");
		break;
