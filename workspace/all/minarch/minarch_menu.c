/**
 * minarch_menu.c - In-game menu system implementation
 *
...
///////////////////////////////
// All external dependencies from minarch.c are now accessed via
...
static void Menu_init_ctx(MinArchContext* ctx) {
...
static void Menu_quit_ctx(MinArchContext* ctx) {
	MinArchMenuState* m = ctx->menu;
...
static void Menu_afterSleep_ctx(MinArchContext* ctx) {
...
static void Menu_saveState_ctx(MinArchContext* ctx) {
	MinArchMenuState* m = ctx->menu;
...
static void Menu_loadState_ctx(MinArchContext* ctx) {
	MinArchMenuState* m = ctx->menu;
	MinArchCallbacks* cb = ctx->callbacks;
...
static void Menu_scale_ctx(MinArchContext* ctx, SDL_Surface* src, SDL_Surface* dst) {
	GFX_Renderer* r = (GFX_Renderer*)ctx->renderer;
	...
	if (scaling == MINARCH_SCALE_CROPPED && dev_w == HDMI_WIDTH) {
		scaling = MINARCH_SCALE_NATIVE;
	}
	if (scaling == MINARCH_SCALE_NATIVE) {
		rx = r->dst_x;
		ry = r->dst_y;
		rw = r->src_w;
		rh = r->src_h;
	...
	} else if (scaling == MINARCH_SCALE_CROPPED) {
		sw -= r->src_x * 2;
		sh -= r->src_y * 2;
	...
	if (scaling == MINARCH_SCALE_ASPECT || rw > dw || rh > dh) {
...
static void Menu_beforeSleep(void) {
	Menu_beforeSleep_ctx(MinArchContext_get());
}

static void Menu_afterSleep(void) {
	Menu_afterSleep_ctx(MinArchContext_get());
}

static void Menu_loop_ctx(MinArchContext* ctx) {
	MinArchMenuState* m = ctx->menu;
	GFX_Renderer* r = (GFX_Renderer*)ctx->renderer;
	struct Game* g = ctx->game;
	struct Core* c = ctx->core;
...
void MinArchMenu_scale(MinArchContext* ctx, SDL_Surface* src, SDL_Surface* dst) {
