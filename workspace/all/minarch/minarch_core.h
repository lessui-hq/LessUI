/**
 * minarch_core.h - Core AV info processing utilities
 *
...
#ifndef __MINARCH_CORE_H__
#define __MINARCH_CORE_H__
 ...
 *   // info.size = game.size
 */
void MinArchCore_buildGameInfo(const struct Game* game, struct retro_game_info* out_info);
 ...
 */
void MinArchCore_processAVInfo(const struct retro_system_av_info* av_info,
                               MinArchCoreAVInfo* out_info);
 ...
 *   double ar = MinArchCore_calculateAspectRatio(1.333, 256, 224);
 *   // Returns 1.333
 *
 *   // Core doesn't provide aspect ratio (0)
 *   double ar = MinArchCore_calculateAspectRatio(0, 256, 224);
 *   // Returns 256.0/224.0 = 1.142857
 */
double MinArchCore_calculateAspectRatio(double provided_aspect, unsigned int width,
                                        unsigned int height);

#endif // __MINARCH_CORE_H__
