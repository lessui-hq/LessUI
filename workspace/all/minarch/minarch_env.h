/**
 * minarch_env.h - Environment callback handlers for MinArch
 *
 ...
 *
 * Usage:
 *   The main environment_callback() in minarch.c dispatches to these handlers.
 *   Each handler receives only the state it needs and returns a result.
 *
 * Extraction from minarch.c environment_callback():
...
#ifndef MINARCH_ENV_H
#define MINARCH_ENV_H
 ...
 */
typedef struct MinArchVideoState {
 ...
 */
typedef struct MinArchThrottleInfo {
 ...
 */
typedef struct MinArchEnvCoreInfo {
