/**
 * launcher_navigation.c - Navigation logic for Launcher
 *
...
void LauncherNav_determineAction(const Entry* entry, const char* current_path,
	...
	if (!entry || !out_action) {
		if (out_action) {
			out_action->action = LAUNCHER_NAV_NONE;
		}
		return;
	}
	...
	switch (entry->type) {
	case ENTRY_ROM:
		out_action->action = LAUNCHER_NAV_OPEN_ROM;
	...
	case ENTRY_PAK:
		out_action->action = LAUNCHER_NAV_OPEN_PAK;
		break;

	case ENTRY_DIR:
		out_action->action = LAUNCHER_NAV_OPEN_DIR;
...
void LauncherNav_buildPakCommand(const char* pak_path, char* out_cmd, int cmd_size) {
	...
	// Note: Launcher_buildPakCommand modifies pak_path, so we copy it first
	char pak_copy[512];
	strncpy(pak_copy, pak_path, sizeof(pak_copy) - 1);
	pak_copy[sizeof(pak_copy) - 1] = '\0';
	Launcher_buildPakCommand(out_cmd, cmd_size, pak_copy);
}
...
void LauncherNav_buildRomCommand(const char* rom_path, const char* emu_name, const char* emu_path,
	...
	// Note: Launcher_buildRomCommand modifies paths, so we use copies
	...
	Launcher_buildRomCommand(out_cmd, cmd_size, emu_copy, rom_copy);
}
...
void LauncherNav_openEntry(LauncherContext* ctx, Entry* entry) {
	...
	LauncherNav_determineAction(entry, current_path, NULL, &action); // TODO: pass COLLECTIONS_PATH
	...
	case LAUNCHER_NAV_OPEN_ROM:
		if (ctx->callbacks && ctx->callbacks->queue_next) {
			// In full implementation, this would call LauncherNav_openRom
			// For now, just note that we need the full openRom logic
		}
		break;

	case LAUNCHER_NAV_OPEN_PAK:
		LauncherNav_openPak(ctx, action.path);
		break;

	case LAUNCHER_NAV_OPEN_DIR:
		LauncherNav_openDirectory(ctx, action.path, action.auto_launch);
		break;
...
void LauncherNav_openPak(LauncherContext* ctx, const char* path) {
	...
	if (ctx->callbacks->queue_next) {
		char cmd[512];
		LauncherNav_buildPakCommand(path, cmd, sizeof(cmd));
...
void LauncherNav_openDirectory(LauncherContext* ctx, const char* path, int auto_launch) {
	...
	if (auto_launch) {
		...
		if (LauncherNav_shouldAutoLaunch(path, launch_path, sizeof(launch_path))) {
			// Would call LauncherNav_openRom with launch_path
...
void LauncherNav_closeDirectory(LauncherContext* ctx) {
	...
	// For now, this is called from launcher.c which has access to the struct
