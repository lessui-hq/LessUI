/**
 * launcher_directory.h - Directory building utilities for Launcher
 *
 ...
 *
 * Extracted from launcher.c for testability with explicit path parameters.
 */

#ifndef __LAUNCHER_DIRECTORY_H__
#define __LAUNCHER_DIRECTORY_H__
 ...
 */
#define LAUNCHER_DIR_MAX_PATH 512
 ...
 * 1. The emulator .pak exists (checked via Launcher_hasEmu)
 ...
 * @param out_prefix Buffer to receive prefix (at least LAUNCHER_DIR_MAX_PATH bytes)
 ...
 *
 * @param path Full path to check
 * @param collation_prefix Prefix from LauncherDir_buildCollationPrefix
 ...
 * is in launcher.c because it depends on global state (simple_mode, recents array).
 *
 ...
 *
 * Note: This is a forward declaration. The actual implementation
 * is in launcher.c because it depends on directory_index module.
 *
