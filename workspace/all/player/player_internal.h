/**
 * player_internal.h - Internal type definitions shared between player modules
 *
 * This header contains struct definitions for internal Player types that
 * need to be accessed by multiple modules (player.c, player_menu.c, etc).
 *
 * These types are implementation details and should not be exposed to
 * external code - use player_context.h for public interfaces.
 */
...
// Forward declaration for button mappings (defined in player_input.h)
typedef struct PlayerButtonMapping PlayerButtonMapping;

// Option types defined in player_options.h
...
struct Config {
	...
	PlayerButtonMapping* shortcuts; // Shortcut mappings
	int loaded;
	int initialized;
};
