/**
 * player_archive.c - Archive extraction utilities using 7z
 *
...
int PlayerArchive_findMatch(const char* archive_path, char* const* extensions, char* out_filename,
	...
	if (!archive_path || !extensions || !out_filename || filename_size == 0)
		return PLAYER_ARCHIVE_ERR_OPEN;
	...
	if (!pipe) {
		...
		return PLAYER_ARCHIVE_ERR_LIST;
	}
	...
	while (fgets(line, sizeof(line), pipe)) {
		...
		if (strncmp(line, "Path = ", 7) == 0) {
			...
			if (PlayerGame_matchesExtension(filename, extensions)) {
				...
				if (strlen(base) >= filename_size) {
					...
					pclose(pipe);
					return PLAYER_ARCHIVE_ERR_NO_MATCH;
				}
...
int PlayerArchive_extract(const char* archive_path, char* const* extensions,
	...
	int result = PlayerArchive_findMatch(archive_path, extensions, filename, sizeof(filename));
	if (result != PLAYER_ARCHIVE_OK)
		return result;

	// Create temp directory
	char tmp_template[MAX_PATH];
	SAFE_STRCPY(tmp_template, "/tmp/player-XXXXXX");
	char* tmp_dirname = mkdtemp(tmp_template);
	if (!tmp_dirname) {
		LOG_error("Failed to create temp directory: %s", strerror(errno));
		return PLAYER_ARCHIVE_ERR_TMPDIR;
	}
	...
	if (!escapeShellArg(archive_path, escaped_archive, sizeof(escaped_archive)) ||
		...
		rmdir(tmp_dirname);
		return PLAYER_ARCHIVE_ERR_EXTRACT;
	}
