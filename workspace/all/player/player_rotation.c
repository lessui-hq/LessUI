/**
 * player_rotation.c - Software rotation buffer management for Player
 *
...
static PlayerRotationBuffer rotation_buffer = {NULL, 0, 0, 0, 0};
...
void PlayerRotation_allocBuffer(uint32_t width, uint32_t height, uint32_t pitch) {
	...
	LOG_debug("PlayerRotation_allocBuffer: %ux%u pitch=%u (need %zu bytes, have %zu)", width,
...
void* PlayerRotation_apply(unsigned rotation, void* src, uint32_t src_w, uint32_t src_h,
	...
	LOG_debug("PlayerRotation_apply: rot=%u, src=%ux%u (pitch=%u) -> dst=%ux%u (pitch=%u)",
	          rotation, src_w, src_h, src_p, dst_w, dst_h, dst_p);

	// Allocate rotation buffer if needed
	PlayerRotation_allocBuffer(dst_w, dst_h, dst_p);
