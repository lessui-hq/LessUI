/**
 * player_scaler.c - Video scaling calculation utilities
 *
 * Pure functions for calculating video scaling parameters.
 * Extracted from player.c selectScaler() for testability.
 */
 ...
 */
static void calculate_native_cropped(const PlayerScalerInput* input, PlayerScalerResult* result,
                                     int src_w, int src_h, PlayerScalerMode mode) {
 ...
 */
static void calculate_fit_mode(const PlayerScalerInput* input, PlayerScalerResult* result,
 ...
 */
static void calculate_oversized(const PlayerScalerInput* input, PlayerScalerResult* result,
...
void PlayerScaler_calculate(const PlayerScalerInput* input, PlayerScalerResult* result) {
	...
	PlayerScaler_applyRotation(input->rotation, &src_w, &src_h);
	...
	PlayerScaler_calculateAspectDimensions(src_w, src_h, aspect_ratio, &aspect_w, &aspect_h);

	// Determine effective scaling mode
	PlayerScalerMode mode = input->mode;
	...
	if (input->buffer_w > 0 && input->buffer_h > 0) {
		PlayerScaler_clampToBuffer(result, input->buffer_w, input->buffer_h, input->bpp);
	}
