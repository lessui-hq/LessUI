/**
 * player_cpu.c - Auto CPU scaling utilities
 *
...
void PlayerCPU_initConfig(PlayerCPUConfig* config) {
	config->window_frames = PLAYER_CPU_DEFAULT_WINDOW_FRAMES;
	config->util_high = PLAYER_CPU_DEFAULT_UTIL_HIGH;
	config->util_low = PLAYER_CPU_DEFAULT_UTIL_LOW;
	config->boost_windows = PLAYER_CPU_DEFAULT_BOOST_WINDOWS;
	config->reduce_windows = PLAYER_CPU_DEFAULT_REDUCE_WINDOWS;
	config->startup_grace = PLAYER_CPU_DEFAULT_STARTUP_GRACE;
	config->min_freq_khz = PLAYER_CPU_DEFAULT_MIN_FREQ_KHZ;
	config->target_util = PLAYER_CPU_DEFAULT_TARGET_UTIL;
	config->max_step_down = PLAYER_CPU_DEFAULT_MAX_STEP_DOWN;
	config->panic_step_up = PLAYER_CPU_DEFAULT_PANIC_STEP_UP;
}

void PlayerCPU_initState(PlayerCPUState* state) {
	memset(state, 0, sizeof(PlayerCPUState));
...
void PlayerCPU_detectFrequencies(PlayerCPUState* state, const PlayerCPUConfig* config,
	...
	state->freq_count = 0;
	for (int i = 0; i < raw_count && state->freq_count < PLAYER_CPU_MAX_FREQUENCIES; i++) {
	...
	state->preset_indices[PLAYER_CPU_LEVEL_POWERSAVE] =
	    PlayerCPU_findNearestIndex(state->frequencies, state->freq_count, ps_target);

	// NORMAL: 80% of max
	int normal_target = max_freq * 80 / 100;
	state->preset_indices[PLAYER_CPU_LEVEL_NORMAL] =
	    PlayerCPU_findNearestIndex(state->frequencies, state->freq_count, normal_target);

	// PERFORMANCE: max frequency
	state->preset_indices[PLAYER_CPU_LEVEL_PERFORMANCE] = state->freq_count - 1;
...
void PlayerCPU_recordFrameTime(PlayerCPUState* state, uint64_t frame_time_us) {
	state->frame_times[state->frame_time_index % PLAYER_CPU_FRAME_BUFFER_SIZE] = frame_time_us;
...
uint64_t PlayerCPU_percentile90(const uint64_t* frame_times, int count) {
	...
	if (count > PLAYER_CPU_FRAME_BUFFER_SIZE)
		count = PLAYER_CPU_FRAME_BUFFER_SIZE;

	// Copy and sort
	uint64_t sorted[PLAYER_CPU_FRAME_BUFFER_SIZE];
...
int PlayerCPU_getPresetPercentage(PlayerCPULevel level) {
	switch (level) {
	case PLAYER_CPU_LEVEL_POWERSAVE:
		return 55;
	case PLAYER_CPU_LEVEL_NORMAL:
		return 80;
	case PLAYER_CPU_LEVEL_PERFORMANCE:
	default:
		return 100;
	}
}
...
PlayerCPUDecision PlayerCPU_update(PlayerCPUState* state, const PlayerCPUConfig* config,
	...
	if (result) {
		result->decision = PLAYER_CPU_DECISION_NONE;
	...
	if (current_underruns > state->last_underrun && !at_max) {
		...
		return PLAYER_CPU_DECISION_PANIC;
	}
	...
	if (state->frame_count < config->window_frames) {
		return PLAYER_CPU_DECISION_NONE;
	}

	// Calculate 90th percentile frame time
	int samples = state->frame_time_index;
	if (samples > PLAYER_CPU_FRAME_BUFFER_SIZE)
		samples = PLAYER_CPU_FRAME_BUFFER_SIZE;
	...
	uint64_t p90_time = PlayerCPU_percentile90(state->frame_times, samples);
	...
	if (state->use_granular) {
		...
		if (util > config->util_high) {
			...
			if (state->high_util_windows >= config->boost_windows && current_idx < max_idx) {
				...
				int needed_freq =
				    PlayerCPU_predictFrequency(current_freq, util, config->target_util);
				int new_idx =
				    PlayerCPU_findNearestIndex(state->frequencies, state->freq_count, needed_freq);
