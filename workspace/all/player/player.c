/**
 * player.c - Launcher Libretro Frontend
 *
 * Player is a lightweight, single-purpose libretro frontend that loads and runs
 ...
 * - Integrates with Launcher launcher via file-based IPC
 *
 * Save State System:
...
// Video geometry state for dynamic updates (type defined in player_env.h)
static PlayerVideoState video_state;

// Video Scaling Modes - defined in player_mappings.h
...
// Video Settings
static int screen_scaling = PLAYER_SCALE_ASPECT; // Default to aspect-ratio preserving
...
static int gamepad_type = 0; // Index in player_gamepad_labels/player_gamepad_values
...
// Core instance (struct defined in player_internal.h)
...
// Game instance (struct defined in player_internal.h)
 ...
 * @note For archives, extracts to /tmp/player-XXXXXX/ (deleted on close)
 */
static void Game_open(char* path) {
	...
	if (PlayerArchive_isArchive(game.path)) {
		...
		PlayerGame_parseExtensions(exts, extensions, PLAYER_MAX_EXTENSIONS);
		...
		if (!core_handles_this_archive) {
			int result =
			    PlayerArchive_extract(game.path, extensions, game.tmp_path, sizeof(game.tmp_path));
			if (result != PLAYER_ARCHIVE_OK) {
				if (result == PLAYER_ARCHIVE_ERR_NO_MATCH) {
	...
	if (PlayerGame_detectM3uPath(game.path, m3u_path, sizeof(m3u_path))) {
 ...
 *
 * @note Writes path to CHANGE_DISC_PATH for Launcher launcher integration
...
void Game_changeDisc(char* path) {
	...
	putFile(CHANGE_DISC_PATH, path); // Launcher still needs to know this to update recents.txt
}
...
static void SRAM_read(void) {
	...
	PlayerMemoryResult result =
	    PlayerMemory_readSRAM(filename, core.get_memory_size, core.get_memory_data);
	if (result != PLAYER_MEM_OK && result != PLAYER_MEM_FILE_NOT_FOUND &&
	    result != PLAYER_MEM_NO_SUPPORT) {
		LOG_error("Error reading SRAM: %s", PlayerMemory_resultString(result));
	}
}
...
void SRAM_write(void) {
	...
	PlayerMemoryResult result =
	    PlayerMemory_writeSRAM(filename, core.get_memory_size, core.get_memory_data);
...
static void RTC_read(void) {
	...
	PlayerMemoryResult result =
	    PlayerMemory_readRTC(filename, core.get_memory_size, core.get_memory_data);
	if (result != PLAYER_MEM_OK && result != PLAYER_MEM_FILE_NOT_FOUND &&
	    result != PLAYER_MEM_NO_SUPPORT) {
		LOG_error("Error reading RTC: %s", PlayerMemory_resultString(result));
	}
}
...
void RTC_write(void) {
	...
	PlayerMemoryResult result =
	    PlayerMemory_writeRTC(filename, core.get_memory_size, core.get_memory_data);
	if (result != PLAYER_MEM_OK && result != PLAYER_MEM_NO_SUPPORT) {
...
// Save State System
///////////////////////////////////////
// Player provides 10 save state slots (0-9):
...
void State_read(void) {
	...
	PlayerStateCore state_core = {.serialize_size = core.serialize_size,
	                              .serialize = core.serialize,
	                              .unserialize = core.unserialize};

	PlayerStateResult result = PlayerState_read(filename, &state_core);
	...
	if (result != PLAYER_STATE_OK && result != PLAYER_STATE_NO_SUPPORT) {
		...
		if (result != PLAYER_STATE_FILE_NOT_FOUND || state_slot != 8) {
			LOG_error("Error reading state: %s (%s)", filename, PlayerState_resultString(result));
		}
	}
...
void State_write(void) {
	...
	PlayerStateResult result = PlayerState_write(filename, &state_core);
 ...
 *
 * Launcher launcher can request a specific slot via RESUME_SLOT_PATH file.
...
// Option and PlayerOptionList types defined in player_internal.h

// Label arrays, enums, and button constants now in player_mappings.h

// Static button mapping arrays now in player_mappings.h/c
// Runtime button mapping state
static PlayerButtonMapping core_button_mapping[RETRO_BUTTON_COUNT + 1] = {0};

// Config state enum aliases (actual enum in player_config.h)
#define CONFIG_NONE PLAYER_CONFIG_NONE
#define CONFIG_CONSOLE PLAYER_CONFIG_CONSOLE
#define CONFIG_GAME PLAYER_CONFIG_GAME
...
// Config instance (struct defined in player_internal.h)
static struct Config config =
    {
        ...
        .frontend =
            {
                ...
                .count = FE_OPT_COUNT,
                .changed = 0,
                .options = (PlayerOption[]){[FE_OPT_SCALING] =
                                                {
                                                    .key = "player_screen_scaling",
                                                    .name = "Screen Scaling",
                                                    .desc =
                                                        NULL, // will call getScreenScalingDesc()
                                                    .full = NULL,
                                                    .var = NULL,
                                                    .default_value = 1,
                                                    .value = 1,
                                            ...
                                            [FE_OPT_EFFECT] =
                                                {
                                                    .key = "player_screen_effect",
                                                    .name = "Screen Effect",
                                                    .desc =
                                                        "Simulates classic CRT and LCD displays.\n"
                                            ...
                                            [FE_OPT_OVERCLOCK] =
                                                {
                                                    .key = "player_cpu_speed",
                                                    .name = "CPU Speed",
                                                    .desc = "Over- or underclock the CPU to "
                                                            "prioritize\n"
                                                            "performance or power savings.\n"
                                                            "Auto adjusts based on emulation "
                                                            "demand.",
                                                    .full = NULL,
                                                    .var = NULL,
                                            ...
                                            [FE_OPT_MAXFF] =
                                                {
                                                    .key = "player_max_ff_speed",
                                                    .name = "Max FF Speed",
                                                    .desc = "Fast forward will not exceed "
                                                            "the\nselected "
                                                            "speed (but may "
                                                            "be less\ndepending on game and "
                                                            "emulator).",
                                                    .full = NULL,
                                                    .var = NULL,
                                                    .default_value = 3, // 4x
                                            ...
                                            [FE_OPT_COUNT] =
                                                {.key = NULL, .name = NULL, .desc = NULL, .full = NULL, .var = NULL, .default_value = 0, .value = 0, .count = 0, .lock = 0, .values = NULL, .labels = NULL}},
                .enabled_count = 0,
        ...
        .controls = player_default_button_mapping,
        .shortcuts = (PlayerButtonMapping[]){[SHORTCUT_SAVE_STATE] = {.name = "Save State",
                                                                      .retro_id = -1,
...
// Config_getValue moved to player_config.c as PlayerConfig_getValue
...
static void Config_syncFrontend(char* key, int value) {
	...
	if (exactMatch(key, config.frontend.options[FE_OPT_SCALING].key)) {
		...
		if (screen_scaling == PLAYER_SCALE_NATIVE || screen_scaling == PLAYER_SCALE_CROPPED)
	...
	if (i == -1)
		return;
	PlayerOption* option = &config.frontend.options[i];
	option->value = value;
}
static void PlayerOptionList_setOptionValue(PlayerOptionList* list, const char* key,
                                            const char* value);
enum {
	CONFIG_WRITE_ALL,
	CONFIG_WRITE_GAME,
...
// Configuration stored in /mnt/SDCARD/.userdata/<platform>/<core>/player.cfg
...
static void Config_init(void) {
	...
	while ((tmp = strstr(tmp, "bind "))) {
		...
		int remap = 0;
		if (tmp2) {
			const PlayerButtonMapping* found =
			    PlayerInput_findMappingByName(player_button_label_mapping, tmp2 + 1);
			if (found)
				retro_id = found->retro_id;
			*tmp2 = '\0';
		}
		const PlayerButtonMapping* found =
		    PlayerInput_findMappingByName(player_button_label_mapping, button_id);
		if (found) {
...
static void Config_readOptionsString(char* cfg) {
	...
	for (int i = 0; config.frontend.options[i].key; i++) {
		PlayerOption* option = &config.frontend.options[i];
		if (!PlayerConfig_getValue(cfg, option->key, value, &option->lock))
			continue;
		PlayerOptionList_setOptionValue(&config.frontend, option->key, value);
		Config_syncFrontend(option->key, option->value);
	}

	if (has_custom_controllers && PlayerConfig_getValue(cfg, "player_gamepad_type", value, NULL)) {
		gamepad_type = strtol(value, NULL, 0);
		int device = strtol(player_gamepad_values[gamepad_type], NULL, 0);
		core.set_controller_port_device(0, device);
	}

	for (int i = 0; config.core.options[i].key; i++) {
		PlayerOption* option = &config.core.options[i];
		if (!PlayerConfig_getValue(cfg, option->key, value, &option->lock))
			continue;
		PlayerOptionList_setOptionValue(&config.core, option->key, value);
	}
}
static void Config_readControlsString(char* cfg) {
	...
	for (int i = 0; config.controls[i].name; i++) {
		...
		int id = -1;
		for (int j = 0; player_button_labels[j]; j++) {
			if (!strcmp(player_button_labels[j], value)) {
				id = j - 1;
				break;
			}
		}
...
static void Config_readOptions(void) {
	...
	// screen_scaling = PLAYER_SCALE_NATIVE; // TODO: tmp
}
...
static void Config_write(int override) {
	...
	if (has_custom_controllers)
		(void)fprintf(file, "%s = %i\n", "player_gamepad_type", gamepad_type);
	...
	for (int i = 0; config.controls[i].name; i++) {
		...
		(void)fprintf(file, "bind %s = %s\n", mapping->name, player_button_labels[j]);
	}
	for (int i = 0; config.shortcuts[i].name; i++) {
		PlayerButtonMapping* mapping = &config.shortcuts[i];
		int j = mapping->local_id + 1;
...
static int PlayerOption_getValueIndex(PlayerOption* item, const char* value) {
	if (!value)
		return 0;
...
static void PlayerOption_setValue(PlayerOption* item, const char* value) {
	// TODO: store previous value?
	item->value = PlayerOption_getValueIndex(item, value);
}

// TODO: does this also need to be applied to PlayerOptionList_vars()?
...
static void PlayerOptionList_init(const struct retro_core_option_definition* defs) {
	...
	if (count) {
		config.core.options = calloc(count + 1, sizeof(PlayerOption));

		for (int i = 0; i < config.core.count; i++) {
			int len;
			const struct retro_core_option_definition* def = &defs[i];
			PlayerOption* item = &config.core.options[i];
...
static void PlayerOptionList_vars(const struct retro_variable* vars) {
	LOG_debug("PlayerOptionList_vars");
	int count;
...
static void PlayerOptionList_reset(void) {
...
static PlayerOption* PlayerOptionList_getOption(PlayerOptionList* list, const char* key) {
	for (int i = 0; i < list->count; i++) {
		PlayerOption* item = &list->options[i];
		if (!strcmp(item->key, key))
			return item;
	}
	return NULL;
}
static char* PlayerOptionList_getOptionValue(PlayerOptionList* list, const char* key) {
...
static void PlayerOptionList_setOptionRawValue(PlayerOptionList* list, const char* key, int value) {
	PlayerOption* item = PlayerOptionList_getOption(list, key);
	if (item) {
		item->value = value;
		list->changed = 1;
...
static void PlayerOptionList_setOptionValue(PlayerOptionList* list, const char* key,
	...
	PlayerOption* item = PlayerOptionList_getOption(list, key);
	if (item) {
		PlayerOption_setValue(item, value);
		list->changed = 1;
...
static void Input_init(const struct retro_input_descriptor* vars) {
	...
	config.controls =
	    core_button_mapping[0].name ? core_button_mapping : player_default_button_mapping;
	...
	for (int i = 0; player_default_button_mapping[i].name; i++) {
		PlayerButtonMapping* mapping = &player_default_button_mapping[i];
		LOG_debug("DEFAULT %s (%s): <%s>", core_button_names[mapping->retro_id], mapping->name,
		          (mapping->local_id == BTN_ID_NONE
		               ? "NONE"
		               : player_device_button_names[mapping->local_id]));
...
static bool environment_callback(unsigned cmd, void* data) { // copied from picoarch initially
	...
	case RETRO_ENVIRONMENT_SET_ROTATION: { /* 1 */
		result = PlayerEnv_setRotation(&video_state, data);
	...
	case RETRO_ENVIRONMENT_SET_PIXEL_FORMAT: { /* 10 */
		result = PlayerEnv_setPixelFormat(&pixel_format, data);
		return result.success;
	}
	...
	case RETRO_ENVIRONMENT_GET_VARIABLE: { /* 15 */
		...
		if (var && var->key) {
			var->value = PlayerOptionList_getOptionValue(&config.core, var->key);
			// printf("\t%s = %s\n", var->key, var->value);
		}
		// fflush(stdout);
		break;
	...
	case RETRO_ENVIRONMENT_SET_VARIABLES: { /* 16 */
		...
		if (vars) {
			PlayerOptionList_reset();
			PlayerOptionList_vars(vars);
		}
		break;
	}
	...
	case RETRO_ENVIRONMENT_SET_FRAME_TIME_CALLBACK: { /* 21 */
		result = PlayerEnv_setFrameTimeCallback(&video_state, data);
		return result.success;
	}
	...
	case RETRO_ENVIRONMENT_GET_AUDIO_VIDEO_ENABLE: {
		...
		PlayerEnv_getAudioVideoEnable(data);
		break;
	}
	...
	case RETRO_ENVIRONMENT_GET_FASTFORWARDING: { /* 49 */
		result = PlayerEnv_getFastforwarding(fast_forward, data);
		return result.success;
	}
	case RETRO_ENVIRONMENT_GET_TARGET_REFRESH_RATE: { /* 50 | RETRO_ENVIRONMENT_EXPERIMENTAL */
		result = PlayerEnv_getTargetRefreshRate(core.fps, data);
		return result.success;
	}
	...
	case RETRO_ENVIRONMENT_SET_CORE_OPTIONS: { /* 53 */
		...
		if (data) {
			PlayerOptionList_reset();
			PlayerOptionList_init((const struct retro_core_option_definition*)data);
		}
		break;
	}
	case RETRO_ENVIRONMENT_SET_CORE_OPTIONS_INTL: { /* 54 */
		...
		if (options && options->us) {
			PlayerOptionList_reset();
			PlayerOptionList_init(options->us);
		}
		break;
	}
...
// Pixel format conversion - implementation in player_video_convert.c
static void pixel_convert(const void* data, unsigned width, unsigned height, size_t pitch) {
	PlayerVideoConvert_convert(data, width, height, pitch, (PlayerPixelFormat)pixel_format);
}
...
static void* apply_rotation(void* src, uint32_t src_w, uint32_t src_h, uint32_t src_p) {
	return PlayerRotation_apply(video_state.rotation, src, src_w, src_h, src_p);
}
 ...
 *
 * Scaling modes:
 * - PLAYER_SCALE_NATIVE: 1:1 pixel mapping (may be cropped if game > screen)
 * - PLAYER_SCALE_ASPECT: Maintain aspect ratio with letterboxing
 * - SCALE_FULLSCREEN: Stretch to fill screen (may distort)
 * - PLAYER_SCALE_CROPPED: Crop to fill screen while maintaining aspect
 *
...
void selectScaler(int src_w, int src_h, int src_p) {
	if (NEEDS_CONVERSION)
		PlayerVideoConvert_allocBuffer(src_w, src_h);

	// Build input parameters for scaler calculation
	PlayerScalerInput input = {.src_w = src_w,
	                           .src_h = src_h,
	                           .src_p = src_p,
	                           .aspect_ratio = core.aspect_ratio,
	                           .rotation = (PlayerRotation)video_state.rotation,
	                           .mode = (PlayerScalerMode)screen_scaling,
...
static void video_refresh_callback_main(const void* data, unsigned width, unsigned height,
	...
	if (NEEDS_CONVERSION) {
		...
		frame_data = PlayerVideoConvert_getBuffer();
	...
	if (rotated_data != frame_data) {
		renderer.src_p = PlayerRotation_getBuffer()->pitch;
	}
...
bool Core_load(void) {
	LOG_info("Core_load");
	struct retro_game_info game_info;
	PlayerCore_buildGameInfo(&game, &game_info);
	...
	PlayerCore_processAVInfo(&av_info, &info);
	core.fps = info.fps;
...
///////////////////////////////////////
// Menu init/quit wrappers (implementation in player_menu.c)
///////////////////////////////////////

void Menu_init(void) {
	PlayerMenu_init(PlayerContext_get());
}

void Menu_quit(void) {
	PlayerMenu_quit(PlayerContext_get());
}

///////////////////////////////////////
// Sleep handlers - delegate to player_menu.c (which uses callbacks)
///////////////////////////////////////

void Menu_beforeSleep(void) {
	PlayerMenu_beforeSleep(PlayerContext_get());
}

void Menu_afterSleep(void) {
	PlayerMenu_afterSleep(PlayerContext_get());
}

// Menu types defined in player_menu_types.h
...
static int OptionsMenu_buildAndShow(PlayerOptionList* source, MenuList* menu,
	...
	if (!source->enabled_count) {
		...
		source->enabled_options = calloc(enabled_count + 1, sizeof(PlayerOption*));
		int j = 0;
		for (int idx = 0; idx < source->count; idx++) {
...
static int OptionEmulator_optionChanged(MenuList* list, int i) {
	...
	PlayerOption* option = PlayerOptionList_getOption(&config.core, item->key);
	if (option) {
...
int OptionControls_bind(MenuList* list, int i) {
	MenuItem* item = &list->items[i];
	if (item->values != player_button_labels) {
		// LOG_info("changed gamepad_type");
		return MENU_CALLBACK_NOP;
	}

	PlayerButtonMapping* button = &config.controls[item->id];
...
static int OptionControls_optionChanged(MenuList* list, int i) {
	MenuItem* item = &list->items[i];
	if (item->values != player_gamepad_labels)
		return MENU_CALLBACK_NOP;

	if (has_custom_controllers) {
		gamepad_type = item->value;
		int device = strtol(player_gamepad_values[item->value], NULL, 0);
		core.set_controller_port_device(0, device);
	}
	return MENU_CALLBACK_NOP;
}
...
static int OptionControls_openMenu(MenuList* list, int i) {
	...
	if (OptionControls_menu.items == NULL) {
		...
		for (int j = 0; config.controls[j].name; j++) {
			...
			if (button->modifier)
				item->value += LOCAL_BUTTON_COUNT;
			item->values = player_button_labels;
		}
	} else {
		// update values
		int k = 0;
...
static int OptionShortcuts_bind(MenuList* list, int i) {
	MenuItem* item = &list->items[i];
	PlayerButtonMapping* button = &config.shortcuts[item->id];
	int bound = 0;
	while (!bound) {
		GFX_startFrame();
		PAD_poll();
...
static char* getSaveDesc(void) {
	return (char*)PlayerConfig_getStateDesc(config.loaded);
}
static int OptionShortcuts_openMenu(MenuList* list, int i) {
	if (OptionShortcuts_menu.items == NULL) {
		...
		for (int j = 0; config.shortcuts[j].name; j++) {
			PlayerButtonMapping* button = &config.shortcuts[j];
...
MenuList options_menu = {.type = MENU_LIST,
                         ...
                         .items =
                             (MenuItem[]){
                                 {.name = "Frontend",
                                  .desc = "Player (" BUILD_DATE " " BUILD_HASH ")",
                                  .values = NULL,
                                  .key = NULL,
                                  .id = 0,
                                  .value = 0,
...
int Menu_options(MenuList* list) {
	...
	while (show_options) {
		...
		MenuItem* item = &items[nav.selected];
		PlayerMenuAction action = PlayerMenuNav_getAction(
		    list, item, type, PAD_justPressed(BTN_A), PAD_justPressed(BTN_B),
		    PAD_justPressed(BTN_X), player_button_labels);
...
///////////////////////////////
// Menu state wrappers (implementation in player_menu.c)
///////////////////////////////

static void Menu_initState(void) {
	PlayerMenu_initState(PlayerContext_get());
}

static void Menu_updateState(void) {
	PlayerMenu_updateState(PlayerContext_get());
}

static void Menu_saveState(void) {
	PlayerMenu_saveState(PlayerContext_get());
}
...
static void Menu_scale(SDL_Surface* src, SDL_Surface* dst) {
	PlayerMenu_scale(PlayerContext_get(), src, dst);
}
...
static void Menu_loop(void) {
	PlayerMenu_loop(PlayerContext_get());
}
...
int main(int argc, char* argv[]) {
	...
	// Initialize callbacks for menu module to call back into player.c
	// This eliminates extern dependencies in player_menu.c
