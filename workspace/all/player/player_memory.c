/**
 * player_memory.c - SRAM and RTC persistence utilities
 *
 * Provides functions to read/write battery-backed save RAM and
 * real-time clock data to/from disk.
 *
 * Extracted from player.c for testability.
 */

#include "player_memory.h"
#include <errno.h>
#include <stdio.h>

PlayerMemoryResult PlayerMemory_read(const char* filepath, unsigned memory_type,
                                     PlayerGetMemorySizeFn get_size,
                                     PlayerGetMemoryDataFn get_data) {
	// Check if core supports this memory type
	size_t mem_size = get_size(memory_type);
	if (!mem_size) {
		return PLAYER_MEM_NO_SUPPORT;
	}

	// Try to open the file
	FILE* file = fopen(filepath, "rb");
	if (!file) {
		// File not found is a normal case (no save exists yet)
		return PLAYER_MEM_FILE_NOT_FOUND;
	}

	// Get memory pointer from core
	void* mem_data = get_data(memory_type);
	if (!mem_data) {
		(void)fclose(file); // File opened for reading
		return PLAYER_MEM_NULL_POINTER;
	}

	// Read data into memory
	size_t bytes_read = fread(mem_data, 1, mem_size, file);
	(void)fclose(file); // File opened for reading

	// Allow partial reads (some games save different sizes)
	if (bytes_read == 0) {
		return PLAYER_MEM_FILE_ERROR;
	}

	return PLAYER_MEM_OK;
}

PlayerMemoryResult PlayerMemory_write(const char* filepath, unsigned memory_type,
                                      PlayerGetMemorySizeFn get_size,
                                      PlayerGetMemoryDataFn get_data) {
	// Check if core supports this memory type
	size_t mem_size = get_size(memory_type);
	if (!mem_size) {
		return PLAYER_MEM_NO_SUPPORT;
	}

	// Get memory pointer from core
	void* mem_data = get_data(memory_type);
	if (!mem_data) {
		return PLAYER_MEM_NULL_POINTER;
	}

	// Open file for writing
	FILE* file = fopen(filepath, "wb");
	if (!file) {
		return PLAYER_MEM_FILE_ERROR;
	}

	// Write data to file
	size_t bytes_written = fwrite(mem_data, 1, mem_size, file);
	(void)fclose(file); // File opened for writing

	if (bytes_written != mem_size) {
		return PLAYER_MEM_FILE_ERROR;
	}

	return PLAYER_MEM_OK;
}

PlayerMemoryResult PlayerMemory_readSRAM(const char* filepath, PlayerGetMemorySizeFn get_size,
                                         PlayerGetMemoryDataFn get_data) {
	return PlayerMemory_read(filepath, PLAYER_MEMORY_SAVE_RAM, get_size, get_data);
}

PlayerMemoryResult PlayerMemory_writeSRAM(const char* filepath, PlayerGetMemorySizeFn get_size,
                                          PlayerGetMemoryDataFn get_data) {
	return PlayerMemory_write(filepath, PLAYER_MEMORY_SAVE_RAM, get_size, get_data);
}

PlayerMemoryResult PlayerMemory_readRTC(const char* filepath, PlayerGetMemorySizeFn get_size,
                                        PlayerGetMemoryDataFn get_data) {
	return PlayerMemory_read(filepath, PLAYER_MEMORY_RTC, get_size, get_data);
}

PlayerMemoryResult PlayerMemory_writeRTC(const char* filepath, PlayerGetMemorySizeFn get_size,
                                         PlayerGetMemoryDataFn get_data) {
	return PlayerMemory_write(filepath, PLAYER_MEMORY_RTC, get_size, get_data);
}

const char* PlayerMemory_resultString(PlayerMemoryResult result) {
	switch (result) {
	case PLAYER_MEM_OK:
		return "Success";
	case PLAYER_MEM_NO_SUPPORT:
		return "Core does not support this memory type";
	case PLAYER_MEM_FILE_NOT_FOUND:
		return "File not found";
	case PLAYER_MEM_FILE_ERROR:
		return "File I/O error";
	case PLAYER_MEM_NULL_POINTER:
		return "Core returned null memory pointer";
	case PLAYER_MEM_SIZE_MISMATCH:
		return "File size does not match expected size";
	default:
		return "Unknown error";
	}
}
