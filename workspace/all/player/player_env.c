/**
 * player_env.c - Environment callback handlers for Player
 *
...
EnvResult PlayerEnv_setRotation(PlayerVideoState* state, const void* data) {
	...
	if (state) {
		...
		LOG_info("SET_ROTATION: %u (%s)", *rotation, PlayerEnv_getRotationDesc(*rotation));
	}
...
EnvResult PlayerEnv_setPixelFormat(enum retro_pixel_format* pixel_format, const void* data) {
	...
	case RETRO_PIXEL_FORMAT_RGB565:
		if (pixel_format) {
			*pixel_format = *format;
			LOG_info("Core requested %s format", PlayerEnv_getPixelFormatDesc(*format));
		}
		return EnvResult_ok();
