/**
 * test_player_env.c - Unit tests for environment callback handlers
 *
 * Tests the extracted environment callback handlers in player_env.c.
...
void test_getRotationDesc_normal(void) {
	TEST_ASSERT_EQUAL_STRING("0 (normal)", PlayerEnv_getRotationDesc(0));
}

void test_getRotationDesc_90ccw(void) {
	TEST_ASSERT_EQUAL_STRING("90 CCW", PlayerEnv_getRotationDesc(1));
}

void test_getRotationDesc_180(void) {
	TEST_ASSERT_EQUAL_STRING("180", PlayerEnv_getRotationDesc(2));
}

void test_getRotationDesc_270ccw(void) {
	TEST_ASSERT_EQUAL_STRING("270 CCW", PlayerEnv_getRotationDesc(3));
}

void test_getRotationDesc_invalid(void) {
	TEST_ASSERT_EQUAL_STRING("invalid", PlayerEnv_getRotationDesc(4));
	TEST_ASSERT_EQUAL_STRING("invalid", PlayerEnv_getRotationDesc(99));
}
...
void test_getPixelFormatDesc_0rgb1555(void) {
	const char* desc = PlayerEnv_getPixelFormatDesc(RETRO_PIXEL_FORMAT_0RGB1555);
	TEST_ASSERT_NOT_NULL(strstr(desc, "15-bit"));
}

void test_getPixelFormatDesc_xrgb8888(void) {
	const char* desc = PlayerEnv_getPixelFormatDesc(RETRO_PIXEL_FORMAT_XRGB8888);
	TEST_ASSERT_NOT_NULL(strstr(desc, "32-bit"));
}

void test_getPixelFormatDesc_rgb565(void) {
	const char* desc = PlayerEnv_getPixelFormatDesc(RETRO_PIXEL_FORMAT_RGB565);
	TEST_ASSERT_NOT_NULL(strstr(desc, "native"));
}

void test_getPixelFormatDesc_unknown(void) {
	const char* desc = PlayerEnv_getPixelFormatDesc(99);
...
void test_setRotation_valid_values(void) {
	PlayerVideoState state = {0};
	EnvResult result;

	for (unsigned i = 0; i <= 3; i++) {
		result = PlayerEnv_setRotation(&state, &i);
...
void test_setRotation_invalid_value(void) {
	...
	EnvResult result = PlayerEnv_setRotation(&state, &rotation);
	TEST_ASSERT_TRUE(result.handled);
...
void test_setRotation_null_data(void) {
	PlayerVideoState state = {0};
	EnvResult result = PlayerEnv_setRotation(&state, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_setRotation_null_state(void) {
	...
	EnvResult result = PlayerEnv_setRotation(NULL, &rotation);
...
void test_setPixelFormat_0rgb1555(void) {
	...
	EnvResult result = PlayerEnv_setPixelFormat(&format, &requested);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setPixelFormat_null_data(void) {
	enum retro_pixel_format format = RETRO_PIXEL_FORMAT_RGB565;
	EnvResult result = PlayerEnv_setPixelFormat(&format, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_setGeometry_updates_flags(void) {
	...
	EnvResult result = PlayerEnv_setGeometry(&state, &renderer_dst_p, &geometry);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setGeometry_null_data(void) {
	...
	EnvResult result = PlayerEnv_setGeometry(&state, &renderer_dst_p, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_setFrameTimeCallback_registers(void) {
	...
	EnvResult result = PlayerEnv_setFrameTimeCallback(&state, &cb);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setFrameTimeCallback_unregisters(void) {
	PlayerVideoState state = {.frame_time_cb = dummy_frame_time_cb,
...
void test_setFrameTimeCallback_null_data(void) {
	PlayerVideoState state = {0};
	EnvResult result = PlayerEnv_setFrameTimeCallback(&state, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_getSystemDirectory_returns_path(void) {
	...
	EnvResult result = PlayerEnv_getSystemDirectory(bios_dir, &out);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_getSaveDirectory_returns_path(void) {
	...
	EnvResult result = PlayerEnv_getSaveDirectory(saves_dir, &out);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_getFastforwarding_true(void) {
	bool out = false;
	EnvResult result = PlayerEnv_getFastforwarding(1, &out);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_getFastforwarding_false(void) {
	bool out = true;
	EnvResult result = PlayerEnv_getFastforwarding(0, &out);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_getFastforwarding_null_data(void) {
	EnvResult result = PlayerEnv_getFastforwarding(1, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_getTargetRefreshRate_returns_fps(void) {
	float out = 0.0f;
	EnvResult result = PlayerEnv_getTargetRefreshRate(60.0, &out);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_getTargetRefreshRate_null_data(void) {
	EnvResult result = PlayerEnv_getTargetRefreshRate(60.0, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_getAudioVideoEnable_sets_flags(void) {
	int out = 0;
	EnvResult result = PlayerEnv_getAudioVideoEnable(&out);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_getThrottleState_null_throttle(void) {
	struct retro_throttle_state state = {0};
	EnvResult result = PlayerEnv_getThrottleState(NULL, &state);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
}
...
void test_setDiskControlInterface_copies_data(void) {
	...
	EnvResult result = PlayerEnv_setDiskControlInterface(&disk_control, &cb);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setDiskControlExtInterface_copies_data(void) {
	...
	EnvResult result = PlayerEnv_setDiskControlExtInterface(&disk_control, &cb);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setSystemAVInfo_updates_values(void) {
	...
	EnvResult result = PlayerEnv_setSystemAVInfo(&state, &fps, &sample_rate, &aspect_ratio,
...
void test_setSystemAVInfo_calculates_aspect_from_geometry(void) {
	...
	PlayerEnv_setSystemAVInfo(&state, &fps, &sample_rate, &aspect_ratio, &renderer_dst_p, NULL,
	                           &av_info);
...
void test_setAudioBufferStatusCallback_registers(void) {
	...
	EnvResult result = PlayerEnv_setAudioBufferStatusCallback(&cb_ptr, &cb);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setAudioBufferStatusCallback_unregisters(void) {
	...
	EnvResult result = PlayerEnv_setAudioBufferStatusCallback(&cb_ptr, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_TRUE(result.success);
...
void test_setControllerInfo_detects_dualshock(void) {
	...
	EnvResult result = PlayerEnv_setControllerInfo(&has_custom, infos);
	TEST_ASSERT_TRUE(result.handled);
...
void test_setControllerInfo_null_data(void) {
	int has_custom = 0;
	EnvResult result = PlayerEnv_setControllerInfo(&has_custom, NULL);
	TEST_ASSERT_TRUE(result.handled);
	TEST_ASSERT_FALSE(result.success);
	TEST_ASSERT_EQUAL(0, has_custom);
}
