/**
 * test_player_state.c - Unit tests for save state system
 *
...
static PlayerStateCore test_core = {.serialize_size = mock_serialize_size,
...
void test_writeState_writes_to_file(void) {
	...
	PlayerStateResult result = PlayerState_write(test_path, &test_core);

	TEST_ASSERT_EQUAL(PLAYER_STATE_OK, result);
...
void test_writeState_returns_no_support_when_size_zero(void) {
	...
	TEST_ASSERT_EQUAL(PLAYER_STATE_NO_SUPPORT, result);
}
...
void test_writeState_returns_serialize_error_on_fail(void) {
	...
	TEST_ASSERT_EQUAL(PLAYER_STATE_SERIALIZE_ERROR, result);
}
...
void test_readState_loads_from_file(void) {
	...
	PlayerStateResult result = PlayerState_read(test_path, &test_core);
...
void test_readState_returns_file_not_found(void) {
	PlayerStateResult result = PlayerState_read("/nonexistent/path.st0", &test_core);

	TEST_ASSERT_EQUAL(PLAYER_STATE_FILE_NOT_FOUND, result);
}
...
void test_autoSave_saves_to_slot_9(void) {
	...
	PlayerStateResult result = PlayerState_autoSave(test_dir, "TestGame", &test_core);
...
void test_autoSave_uses_correct_slot_number(void) {
	...
	PlayerStateResult result = PlayerState_autoSave(test_dir, "Game", &test_core);
	TEST_ASSERT_EQUAL(PLAYER_STATE_OK, result);

	// Check file exists at auto-resume slot (st9)
	char path[512];
	snprintf(path, sizeof(path), "%s/Game.st%d", test_dir, PLAYER_AUTO_RESUME_SLOT);
	FILE* f = fopen(path, "rb");
...
void test_resumeState_loads_from_specified_slot(void) {
	...
	PlayerStateResult result = PlayerState_resume(test_dir, "TestGame", 3, &test_core);
...
void test_resumeState_returns_file_not_found_for_empty_slot(void) {
	...
	PlayerStateResult result = PlayerState_resume(test_dir, "TestGame", 5, &test_core);
...
void test_resumeState_handles_all_slots(void) {
	...
	for (int slot = 0; slot <= 9; slot++) {
		...
		PlayerStateResult result = PlayerState_resume(test_dir, "Game", slot, &test_core);
		TEST_ASSERT_EQUAL(PLAYER_STATE_OK, result);
...
void test_stateResultString_returns_descriptions(void) {
	TEST_ASSERT_EQUAL_STRING("Success", PlayerState_resultString(PLAYER_STATE_OK));
	TEST_ASSERT_EQUAL_STRING("Core does not support save states",
	                         PlayerState_resultString(PLAYER_STATE_NO_SUPPORT));
	TEST_ASSERT_EQUAL_STRING("State file not found",
	                         PlayerState_resultString(PLAYER_STATE_FILE_NOT_FOUND));
	TEST_ASSERT_EQUAL_STRING("File I/O error", PlayerState_resultString(PLAYER_STATE_FILE_ERROR));
	TEST_ASSERT_EQUAL_STRING("Memory allocation failed",
	                         PlayerState_resultString(PLAYER_STATE_ALLOC_ERROR));
	TEST_ASSERT_EQUAL_STRING("Core serialization failed",
	                         PlayerState_resultString(PLAYER_STATE_SERIALIZE_ERROR));
}
...
void test_state_write_then_read_roundtrip(void) {
	...
	// Read back
	result = PlayerState_read(test_path, &test_core);
	TEST_ASSERT_EQUAL(PLAYER_STATE_OK, result);
...
void test_autosave_then_resume_roundtrip(void) {
	...
	PlayerStateResult result = PlayerState_autoSave(test_dir, "MyGame", &test_core);
	TEST_ASSERT_EQUAL(PLAYER_STATE_OK, result);
	...
	result = PlayerState_resume(test_dir, "MyGame", PLAYER_AUTO_RESUME_SLOT, &test_core);
	TEST_ASSERT_EQUAL(PLAYER_STATE_OK, result);
