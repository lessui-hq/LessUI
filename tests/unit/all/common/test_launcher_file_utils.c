/**
 * test_launcher_file_utils.c - Unit tests for Launcher file utilities
 *
 * Tests file existence checking functions extracted from launcher.c.
...
void test_hasEmu_finds_shared_emulator(void) {
	...
	int result = Launcher_hasEmu("gpsp", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini");
...
void test_hasEmu_finds_platform_specific_emulator(void) {
	...
	int result = Launcher_hasEmu("gambatte", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini");
...
void test_hasEmu_prefers_shared_over_platform(void) {
	...
	int result = Launcher_hasEmu("snes9x", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini");
...
void test_hasEmu_returns_false_when_not_found(void) {
	...
	int result = Launcher_hasEmu("nonexistent", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini");
...
void test_hasEmu_different_platforms(void) {
	...
	TEST_ASSERT_TRUE(Launcher_hasEmu("picodrive", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "rg35xx"));

	// Should NOT find for miyoomini
	TEST_ASSERT_FALSE(
	    Launcher_hasEmu("picodrive", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini"));
}
...
void test_hasCue_finds_matching_cue_file(void) {
	...
	char cue_path[256];
	int result = Launcher_hasCue("/Roms/PS1/Final Fantasy VII", cue_path);
...
void test_hasCue_returns_false_when_cue_missing(void) {
	...
	char cue_path[256];
	int result = Launcher_hasCue("/Roms/PS1/SomeGame", cue_path);
...
void test_hasCue_simple_directory_name(void) {
	...
	char cue_path[256];
	int result = Launcher_hasCue("/Games/Panzer", cue_path);
...
void test_hasCue_path_with_special_characters(void) {
	...
	char cue_path[256];
	int result = Launcher_hasCue("/Roms/Game (USA) (v1.1)", cue_path);
...
void test_hasM3u_finds_m3u_in_parent(void) {
	...
	char m3u_path[256];
	int result = Launcher_hasM3u("/Roms/PS1/FF7/disc1.bin", m3u_path);
...
void test_hasM3u_returns_false_when_m3u_missing(void) {
	char m3u_path[256];
	int result = Launcher_hasM3u("/Roms/PS1/Game/disc.bin", m3u_path);
...
void test_hasM3u_deep_directory_structure(void) {
	...
	char m3u_path[256];
	int result = Launcher_hasM3u("/a/b/c/Game/disc1.bin", m3u_path);
...
void test_hasM3u_realistic_final_fantasy(void) {
	...
	char m3u_path[256];
	int result =
	    Launcher_hasM3u("/mnt/SDCARD/Roms/PS1/Final Fantasy VII (USA)/FF7 (Disc 1).bin", m3u_path);
...
void test_hasM3u_constructs_correct_path_even_when_missing(void) {
	char m3u_path[256];
	Launcher_hasM3u("/Roms/Saturn/Panzer/disc.cue", m3u_path);
...
void test_multi_disc_game_workflow(void) {
	...
	// Check for M3U
	char m3u_path[256];
	TEST_ASSERT_TRUE(Launcher_hasM3u(disc1, m3u_path));
	...
	char cue_path[256];
	TEST_ASSERT_TRUE(Launcher_hasCue(game_dir, cue_path));
}
...
void test_emulator_availability_check(void) {
	...
	TEST_ASSERT_TRUE(Launcher_hasEmu("gpsp", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini"));
	TEST_ASSERT_TRUE(
	    Launcher_hasEmu("gambatte", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini"));
	TEST_ASSERT_FALSE(
	    Launcher_hasEmu("nonexistent", "/mnt/SDCARD/Paks", "/mnt/SDCARD", "miyoomini"));
}
...
void test_hasCue_path_without_slash(void) {
	char cue_path[256];
	int result = Launcher_hasCue("NoSlashes", cue_path);
...
void test_hasM3u_shallow_path(void) {
	char m3u_path[256];
	int result = Launcher_hasM3u("/Game/disc.bin", m3u_path);
...
void test_buildThumbPath_simple_rom(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("/Roms/GB/Tetris.gb", out_path);
...
void test_buildThumbPath_nested_directory(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("/mnt/SDCARD/Roms/SNES/Super Mario World.sfc", out_path);
...
void test_buildThumbPath_folder_entry(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("/Roms/PS1/Final Fantasy VII", out_path);
...
void test_buildThumbPath_null_input(void) {
	...
	int result = Launcher_buildThumbPath(NULL, out_path);
...
void test_buildThumbPath_no_slash(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("NoSlashes.gb", out_path);
...
void test_buildThumbPath_trailing_slash(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("/Roms/GB/", out_path);
...
void test_buildThumbPath_root_file(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("/game.rom", out_path);
...
void test_buildThumbPath_special_characters(void) {
	char out_path[512];
	int result = Launcher_buildThumbPath("/Roms/GBA/Pokemon - Fire Red (USA).gba", out_path);
