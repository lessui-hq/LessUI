/**
 * test_launcher_utils.c - Unit tests for Launcher launcher utilities
 *
 * Tests helper functions extracted from launcher.c.
 * These are pure string logic with no file system dependencies.
 *
 * Test coverage:
 * - Launcher_getIndexChar() - Alphabetical index for quick navigation
 * - Launcher_isConsoleDir() - Console directory classification
 */
...
void test_getIndexChar_lowercase_letters(void) {
	TEST_ASSERT_EQUAL_INT(1, Launcher_getIndexChar("apple"));
	TEST_ASSERT_EQUAL_INT(2, Launcher_getIndexChar("banana"));
	TEST_ASSERT_EQUAL_INT(13, Launcher_getIndexChar("mario"));
	TEST_ASSERT_EQUAL_INT(26, Launcher_getIndexChar("zelda"));
}
...
void test_getIndexChar_numbers_return_zero(void) {
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar("007 GoldenEye"));
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar("1942"));
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar("3D Worldrunner"));
}

void test_getIndexChar_special_chars_return_zero(void) {
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar("!Exclaim"));
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar("@Home"));
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar(".hidden"));
}

void test_getIndexChar_empty_string_returns_zero(void) {
	TEST_ASSERT_EQUAL_INT(0, Launcher_getIndexChar(""));
}

void test_getIndexChar_single_letter(void) {
	TEST_ASSERT_EQUAL_INT(1, Launcher_getIndexChar("A"));
	TEST_ASSERT_EQUAL_INT(26, Launcher_getIndexChar("z"));
}
...
void test_isConsoleDir_top_level_console_directory(void) {
	...
	int result = Launcher_isConsoleDir("/mnt/SDCARD/Roms/GB", "/mnt/SDCARD/Roms");

	TEST_ASSERT_TRUE(result);
}

void test_isConsoleDir_another_console(void) {
	int result = Launcher_isConsoleDir("/mnt/SDCARD/Roms/NES", "/mnt/SDCARD/Roms");
...
void test_isConsoleDir_subdirectory_not_console(void) {
	...
	int result = Launcher_isConsoleDir("/mnt/SDCARD/Roms/GB/Homebrew", "/mnt/SDCARD/Roms");
...
void test_isConsoleDir_deep_subdirectory_not_console(void) {
	int result =
	    Launcher_isConsoleDir("/mnt/SDCARD/Roms/GB/Collections/Action", "/mnt/SDCARD/Roms");

	TEST_ASSERT_FALSE(result);
}

void test_isConsoleDir_non_roms_directory(void) {
	int result = Launcher_isConsoleDir("/mnt/SDCARD/Tools/say.pak", "/mnt/SDCARD/Roms");
...
void test_isConsoleDir_roms_path_itself_not_console(void) {
	...
	int result = Launcher_isConsoleDir("/mnt/SDCARD/Roms", "/mnt/SDCARD/Roms");
...
void test_isConsoleDir_different_roms_path(void) {
	...
	int result = Launcher_isConsoleDir("/sd/Roms/SNES", "/sd/Roms");
...
void test_isConsoleDir_path_without_slash_returns_false(void) {
	...
	int result = Launcher_isConsoleDir("GB", "/mnt/SDCARD/Roms");
...
void test_index_char_for_rom_library(void) {
	...
	for (int i = 0; roms[i] != NULL; i++) {
		TEST_ASSERT_EQUAL_INT(expected[i], Launcher_getIndexChar(roms[i]));
	}
}
...
void test_console_dir_classification(void) {
	...
	TEST_ASSERT_TRUE(Launcher_isConsoleDir("/mnt/SDCARD/Roms/GB", roms));
	TEST_ASSERT_TRUE(Launcher_isConsoleDir("/mnt/SDCARD/Roms/GBA", roms));
	TEST_ASSERT_TRUE(Launcher_isConsoleDir("/mnt/SDCARD/Roms/NES", roms));

	// Not console directories
	TEST_ASSERT_FALSE(Launcher_isConsoleDir("/mnt/SDCARD/Roms/GB/Action", roms));
	TEST_ASSERT_FALSE(Launcher_isConsoleDir("/mnt/SDCARD/Tools", roms));
	TEST_ASSERT_FALSE(Launcher_isConsoleDir("/mnt/SDCARD/Roms", roms));
}
