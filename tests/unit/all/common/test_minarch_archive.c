/**
 * test_minarch_archive.c - Tests for minarch_archive module
 *
 * Tests the archive extraction module that shells out to 7z binary.
 */

#include "../../../support/unity/unity.h"
#include "../../../../workspace/all/minarch/minarch_archive.h"
#include <string.h>
...
///////////////////////////////////////
// MinArchArchive_isArchive Tests
///////////////////////////////////////

void test_isArchive_zip_lowercase(void) {
	TEST_ASSERT_TRUE(MinArchArchive_isArchive("/path/to/game.zip"));
}
...
void test_isArchive_7z_lowercase(void) {
	TEST_ASSERT_TRUE(MinArchArchive_isArchive("/path/to/game.7z"));
}
...
void test_isArchive_not_archive_gb(void) {
	TEST_ASSERT_FALSE(MinArchArchive_isArchive("/path/to/game.gb"));
}

void test_isArchive_not_archive_gba(void) {
	TEST_ASSERT_FALSE(MinArchArchive_isArchive("/path/to/game.gba"));
}

void test_isArchive_not_archive_nes(void) {
	TEST_ASSERT_FALSE(MinArchArchive_isArchive("/path/to/game.nes"));
}

void test_isArchive_not_archive_no_extension(void) {
	TEST_ASSERT_FALSE(MinArchArchive_isArchive("/path/to/game"));
}

void test_isArchive_null_path(void) {
	TEST_ASSERT_FALSE(MinArchArchive_isArchive(NULL));
}

void test_isArchive_empty_path(void) {
	TEST_ASSERT_FALSE(MinArchArchive_isArchive(""));
}

void test_isArchive_partial_match_zip(void) {
	// Should not match if .zip is in middle of filename
	TEST_ASSERT_FALSE(MinArchArchive_isArchive("/path/to/game.zip.txt"));
}

void test_isArchive_partial_match_7z(void) {
	// Should not match if .7z is in middle of filename
	TEST_ASSERT_FALSE(MinArchArchive_isArchive("/path/to/game.7z.txt"));
}

// Note: We don't test MinArchArchive_extract() or MinArchArchive_findMatch()
