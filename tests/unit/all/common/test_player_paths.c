/**
 * test_player_paths.c - Unit tests for Player path generation
 *
 * Tests path generation functions extracted from player.c.
...
void test_getSRAMPath_generates_correct_path(void) {
	...
	PlayerPaths_getSRAM(path, "/mnt/SDCARD/.userdata/miyoomini/gpsp", "Pokemon Red");
...
void test_getSRAMPath_handles_special_characters(void) {
	...
	PlayerPaths_getSRAM(path, "/saves", "Game (USA) (Rev 1)");
...
void test_getSRAMPath_handles_short_names(void) {
	...
	PlayerPaths_getSRAM(path, "/data", "A");
...
void test_getRTCPath_generates_correct_path(void) {
	...
	PlayerPaths_getRTC(path, "/mnt/SDCARD/.userdata/miyoomini/gpsp", "Pokemon Gold");
...
void test_getRTCPath_different_from_SRAM(void) {
	...
	PlayerPaths_getSRAM(sram_path, "/saves", "Game");
	PlayerPaths_getRTC(rtc_path, "/saves", "Game");
...
void test_getStatePath_generates_path_for_slot_0(void) {
	...
	PlayerPaths_getState(path, "/states", "Super Mario", 0);
...
void test_getStatePath_generates_path_for_slot_9(void) {
	...
	PlayerPaths_getState(path, "/states", "Zelda", 9);
...
void test_getStatePath_all_slots_unique(void) {
	...
	for (int i = 0; i < 10; i++) {
		PlayerPaths_getState(paths[i], "/s", "Game", i);
	}
...
void test_getStatePath_handles_long_game_names(void) {
	...
	PlayerPaths_getState(path, "/data", "The Legend of Zelda - A Link to the Past", 3);
...
void test_getConfigPath_global_no_device_tag(void) {
	...
	PlayerConfig_getPath(path, "/config", NULL, NULL);

	TEST_ASSERT_EQUAL_STRING("/config/player.cfg", path);
}

void test_getConfigPath_global_with_device_tag(void) {
	char path[512];

	PlayerConfig_getPath(path, "/config", NULL, "rg35xx");

	TEST_ASSERT_EQUAL_STRING("/config/player-rg35xx.cfg", path);
}

void test_getConfigPath_game_specific_no_device_tag(void) {
	char path[512];

	PlayerConfig_getPath(path, "/config", "Super Mario Bros", NULL);
...
void test_getConfigPath_game_specific_with_device_tag(void) {
	...
	PlayerConfig_getPath(path, "/config", "Metroid", "miyoomini");
...
void test_getConfigPath_empty_device_tag_treated_as_null(void) {
	...
	PlayerConfig_getPath(path1, "/cfg", "Game", NULL);
	PlayerConfig_getPath(path2, "/cfg", "Game", "");
...
void test_all_save_files_in_same_directory(void) {
	...
	PlayerPaths_getSRAM(sram_path, saves_dir, game);
	PlayerPaths_getRTC(rtc_path, saves_dir, game);
	PlayerPaths_getState(state_path, saves_dir, game, 0);
...
void test_config_paths_distinguish_game_and_global(void) {
	...
	PlayerConfig_getPath(game_cfg, "/cfg", "MyGame", NULL);
	PlayerConfig_getPath(global_cfg, "/cfg", NULL, NULL);
	...
	// Global config has "player"
	TEST_ASSERT_TRUE(strstr(global_cfg, "player") != NULL);
}
...
void test_getTagBiosPath_generates_correct_path(void) {
	...
	PlayerPaths_getTagBios("/mnt/SDCARD/Bios", "GB", path);
...
void test_getTagBiosPath_handles_longer_tags(void) {
	...
	PlayerPaths_getTagBios("/Bios", "SEGACD", path);
...
void test_chooseBiosPath_uses_tag_dir_when_has_files(void) {
	...
	PlayerPaths_chooseBios("/mnt/SDCARD/Bios", "PS", path, 1); // tag dir has files
...
void test_chooseBiosPath_falls_back_to_root_when_empty(void) {
	...
	PlayerPaths_chooseBios("/mnt/SDCARD/Bios", "PS", path, 0); // tag dir empty
...
void test_chooseBiosPath_uses_different_paths_based_on_has_files(void) {
	...
	PlayerPaths_chooseBios("/Bios", "N64", path_with_files, 1);
	PlayerPaths_chooseBios("/Bios", "N64", path_without_files, 0);
...
void test_chooseBiosPath_organized_user_scenario(void) {
	...
	PlayerPaths_chooseBios("/mnt/SDCARD/Bios", "GB", gb_bios, 1);
	PlayerPaths_chooseBios("/mnt/SDCARD/Bios", "PS", ps_bios, 1);
...
void test_chooseBiosPath_messy_user_scenario(void) {
	...
	PlayerPaths_chooseBios("/mnt/SDCARD/Bios", "GB", gb_bios, 0);
	PlayerPaths_chooseBios("/mnt/SDCARD/Bios", "PS", ps_bios, 0);
