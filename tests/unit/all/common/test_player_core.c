/**
 * test_player_core.c - Unit tests for core AV info processing
 *
 * Tests the pure functions for processing libretro core audio/video
 * information structures.
 *
 * Test coverage:
 * - PlayerCore_buildGameInfo - Build game info from Game struct
 * - PlayerCore_calculateAspectRatio - Aspect ratio calculation
 * - PlayerCore_processAVInfo - Full AV info processing
 */
...
///////////////////////////////
// PlayerCore_buildGameInfo tests
///////////////////////////////
...
void test_buildGameInfo_with_tmp_path(void) {
	...
	strcpy(game.tmp_path, "/tmp/player-abc123/game.nes");
	...
	TEST_ASSERT_EQUAL_STRING("/tmp/player-abc123/game.nes", info.path);
...
void test_buildGameInfo_null_game(void) {
	...
	PlayerCore_buildGameInfo(NULL, &info);
...
void test_buildGameInfo_null_output(void) {
	...
	// Should not crash
	PlayerCore_buildGameInfo(&game, NULL);
}

///////////////////////////////
// PlayerCore_calculateAspectRatio tests
///////////////////////////////

void test_calculateAspectRatio_provided(void) {
	double result = PlayerCore_calculateAspectRatio(1.5, 256, 224);
	TEST_ASSERT_EQUAL_INT(150, (int)(result * 100));
}

void test_calculateAspectRatio_4_3_provided(void) {
	double result = PlayerCore_calculateAspectRatio(4.0 / 3.0, 320, 240);
...
void test_calculateAspectRatio_zero_calculates(void) {
	double result = PlayerCore_calculateAspectRatio(0, 256, 224);
...
void test_calculateAspectRatio_snes(void) {
	...
	double result = PlayerCore_calculateAspectRatio(4.0 / 3.0, 256, 224);
	TEST_ASSERT_INT_WITHIN(1, 133, (int)(result * 100));
}

void test_calculateAspectRatio_genesis(void) {
	// Genesis: 320x224, no provided aspect
	double result = PlayerCore_calculateAspectRatio(0, 320, 224);
	// 320/224 = 1.428571
...
void test_calculateAspectRatio_gba(void) {
	...
	double result = PlayerCore_calculateAspectRatio(3.0 / 2.0, 240, 160);
	TEST_ASSERT_EQUAL_INT(150, (int)(result * 100));
}

///////////////////////////////
// PlayerCore_processAVInfo tests
///////////////////////////////
...
void test_processAVInfo_null_input(void) {
	PlayerCoreAVInfo info = {.fps = 99.9, .sample_rate = 99999, .aspect_ratio = 9.9};

	PlayerCore_processAVInfo(NULL, &info);
...
void test_processAVInfo_null_output(void) {
	...
	// Should not crash
	PlayerCore_processAVInfo(&av, NULL);
}
