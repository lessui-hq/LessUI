/**
 * test_minui_context.c - Unit tests for MinUI context module
 *
...
///////////////////////////////
// MinUIContext_get Tests
///////////////////////////////

void test_MinUIContext_get_returns_non_null(void) {
	MinUIContext* ctx = MinUIContext_get();
	TEST_ASSERT_NOT_NULL(ctx);
}

void test_MinUIContext_get_returns_same_instance(void) {
	MinUIContext* ctx1 = MinUIContext_get();
	MinUIContext* ctx2 = MinUIContext_get();
	TEST_ASSERT_EQUAL_PTR(ctx1, ctx2);
}

void test_MinUIContext_get_initializes_callbacks_pointer(void) {
	MinUIContext* ctx = MinUIContext_get();
	TEST_ASSERT_NOT_NULL(ctx->callbacks);
}

///////////////////////////////
// MinUIContext_getCallbacks Tests
///////////////////////////////

void test_MinUIContext_getCallbacks_returns_non_null(void) {
	MinUICallbacks* callbacks = MinUIContext_getCallbacks();
	TEST_ASSERT_NOT_NULL(callbacks);
}

void test_MinUIContext_getCallbacks_matches_context_callbacks(void) {
	MinUIContext* ctx = MinUIContext_get();
	MinUICallbacks* callbacks = MinUIContext_getCallbacks();
...
///////////////////////////////
// MinUIContext_initCallbacks Tests
///////////////////////////////

void test_MinUIContext_initCallbacks_sets_callbacks(void) {
	MinUIContext* ctx = MinUIContext_get();
	MinUICallbacks my_callbacks = {0};
	my_callbacks.queue_next = (MinUIQueueNextFunc)0x12345678; // Dummy pointer

	MinUIContext_initCallbacks(ctx, &my_callbacks);

	TEST_ASSERT_EQUAL_PTR(&my_callbacks, ctx->callbacks);
	TEST_ASSERT_EQUAL_PTR((void*)0x12345678, ctx->callbacks->queue_next);
}

void test_MinUIContext_initCallbacks_null_ctx_does_not_crash(void) {
	MinUICallbacks callbacks = {0};
	MinUIContext_initCallbacks(NULL, &callbacks);
	// Should not crash
}

void test_MinUIContext_initCallbacks_null_callbacks_does_not_crash(void) {
	MinUIContext* ctx = MinUIContext_get();
	MinUICallbacks* original = ctx->callbacks;

	MinUIContext_initCallbacks(ctx, NULL);
...
///////////////////////////////
// MinUIContext_initGlobals Tests
///////////////////////////////

void test_MinUIContext_initGlobals_does_not_crash(void) {
	MinUIContext* ctx = MinUIContext_get();
	MinUIContext_initGlobals(ctx);
	// Should not crash - it's a stub
}

void test_MinUIContext_initGlobals_null_does_not_crash(void) {
	MinUIContext_initGlobals(NULL);
	// Should not crash
}
...
int main(void) {
	...
	// MinUIContext_get tests
	RUN_TEST(test_MinUIContext_get_returns_non_null);
	RUN_TEST(test_MinUIContext_get_returns_same_instance);
	RUN_TEST(test_MinUIContext_get_initializes_callbacks_pointer);

	// MinUIContext_getCallbacks tests
	RUN_TEST(test_MinUIContext_getCallbacks_returns_non_null);
	RUN_TEST(test_MinUIContext_getCallbacks_matches_context_callbacks);

	// MinUIContext_initCallbacks tests
	RUN_TEST(test_MinUIContext_initCallbacks_sets_callbacks);
	RUN_TEST(test_MinUIContext_initCallbacks_null_ctx_does_not_crash);
	RUN_TEST(test_MinUIContext_initCallbacks_null_callbacks_does_not_crash);

	// MinUIContext_initGlobals tests
	RUN_TEST(test_MinUIContext_initGlobals_does_not_crash);
	RUN_TEST(test_MinUIContext_initGlobals_null_does_not_crash);
