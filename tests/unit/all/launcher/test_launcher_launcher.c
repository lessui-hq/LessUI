/**
 * test_launcher_launcher.c - Unit tests for ROM/PAK launcher utilities
 *
...
void test_replaceString_replaces_single_occurrence(void) {
	...
	int count = Launcher_replaceString(str, "World", "Universe");
...
void test_replaceString_replaces_multiple_occurrences(void) {
	...
	int count = Launcher_replaceString(str, "one", "1");
...
void test_replaceString_handles_no_match(void) {
	...
	int count = Launcher_replaceString(str, "foo", "bar");
...
void test_replaceString_replaces_with_longer_string(void) {
	...
	int count = Launcher_replaceString(str, " ", "---");
...
void test_replaceString_replaces_with_shorter_string(void) {
	...
	int count = Launcher_replaceString(str, "World", "X");
...
void test_replaceString_replaces_with_empty_string(void) {
	...
	int count = Launcher_replaceString(str, " World", "");
...
void test_replaceString_replaces_at_start(void) {
	...
	int count = Launcher_replaceString(str, "Hello", "Hi");
...
void test_replaceString_replaces_at_end(void) {
	...
	int count = Launcher_replaceString(str, "World", "There");
...
void test_escapeSingleQuotes_escapes_single_quote(void) {
	...
	char* result = Launcher_escapeSingleQuotes(str);
...
void test_escapeSingleQuotes_escapes_multiple_quotes(void) {
	...
	Launcher_escapeSingleQuotes(str);
...
void test_buildPakCommand_creates_correct_format(void) {
	char cmd[LAUNCHER_MAX_CMD_SIZE];
	char pak_path[256] = "/mnt/SDCARD/.system/miyoomini/paks/Input.pak";

	int len = Launcher_buildPakCommand(cmd, sizeof(cmd), pak_path);
...
void test_buildPakCommand_returns_zero_on_null_cmd(void) {
	...
	int len = Launcher_buildPakCommand(NULL, 100, pak_path);
...
void test_buildPakCommand_returns_zero_on_null_path(void) {
	...
	int len = Launcher_buildPakCommand(cmd, sizeof(cmd), NULL);
...
void test_buildRomCommand_creates_correct_format(void) {
	...
	int len = Launcher_buildRomCommand(cmd, sizeof(cmd), emu_path, rom_path);
...
void test_buildRomCommand_returns_zero_on_null_inputs(void) {
	...
	TEST_ASSERT_EQUAL(0, Launcher_buildRomCommand(NULL, 100, path, path));
	TEST_ASSERT_EQUAL(0, Launcher_buildRomCommand(cmd, sizeof(cmd), NULL, path));
	TEST_ASSERT_EQUAL(0, Launcher_buildRomCommand(cmd, sizeof(cmd), path, NULL));
}
...
void test_queueCommand_writes_to_file(void) {
	...
	int result = Launcher_queueCommand(test_path, cmd);
...
void test_queueCommand_returns_error_on_null_inputs(void) {
	TEST_ASSERT_EQUAL(-1, Launcher_queueCommand(NULL, "cmd"));
	TEST_ASSERT_EQUAL(-1, Launcher_queueCommand(test_path, NULL));
}
...
void test_queueCommand_overwrites_existing_file(void) {
	...
	// Queue new command
	int result = Launcher_queueCommand(test_path, "new command");
	TEST_ASSERT_EQUAL(0, result);
...
void test_isRomsPath_returns_true_for_exact_match(void) {
	int result = Launcher_isRomsPath("/mnt/SDCARD/Roms", "/mnt/SDCARD/Roms");
	TEST_ASSERT_EQUAL(1, result);
}

void test_isRomsPath_returns_true_for_subpath(void) {
	int result = Launcher_isRomsPath("/mnt/SDCARD/Roms/GB/game.gb", "/mnt/SDCARD/Roms");
	TEST_ASSERT_EQUAL(1, result);
}

void test_isRomsPath_returns_false_for_different_path(void) {
	int result = Launcher_isRomsPath("/mnt/SDCARD/Apps/something", "/mnt/SDCARD/Roms");
	TEST_ASSERT_EQUAL(0, result);
}

void test_isRomsPath_returns_false_for_similar_prefix(void) {
	// /mnt/SDCARD/RomsExtra should NOT match /mnt/SDCARD/Roms
	int result = Launcher_isRomsPath("/mnt/SDCARD/RomsExtra/game.gb", "/mnt/SDCARD/Roms");
	TEST_ASSERT_EQUAL(0, result);
}

void test_isRomsPath_handles_null_inputs(void) {
	TEST_ASSERT_EQUAL(0, Launcher_isRomsPath(NULL, "/mnt/SDCARD/Roms"));
	TEST_ASSERT_EQUAL(0, Launcher_isRomsPath("/mnt/SDCARD/Roms/game", NULL));
	TEST_ASSERT_EQUAL(0, Launcher_isRomsPath(NULL, NULL));
}

void test_isRomsPath_handles_trailing_slash(void) {
	// Path with trailing slash after roms_path
	int result = Launcher_isRomsPath("/mnt/SDCARD/Roms/GB", "/mnt/SDCARD/Roms");
	TEST_ASSERT_EQUAL(1, result);
}
