/**
 * test_launcher_navigation.c - Unit tests for Launcher navigation module
 *
...
///////////////////////////////
// LauncherNav_determineAction Tests
///////////////////////////////
...
void test_determineAction_rom_entry_returns_open_rom(void) {
	...
	LauncherNavAction action;

	LauncherNav_determineAction(&entry, "/mnt/SDCARD/Roms/GB", NULL, &action);

	TEST_ASSERT_EQUAL(LAUNCHER_NAV_OPEN_ROM, action.action);
...
void test_determineAction_pak_entry_returns_open_pak(void) {
	...
	LauncherNav_determineAction(&entry, "/mnt/SDCARD/Tools", NULL, &action);

	TEST_ASSERT_EQUAL(LAUNCHER_NAV_OPEN_PAK, action.action);
...
void test_determineAction_dir_entry_returns_open_dir_with_auto_launch(void) {
	...
	LauncherNav_determineAction(&entry, "/mnt/SDCARD/Roms/PS1", NULL, &action);

	TEST_ASSERT_EQUAL(LAUNCHER_NAV_OPEN_DIR, action.action);
...
void test_determineAction_null_entry_returns_none(void) {
	LauncherNavAction action;
	action.action = LAUNCHER_NAV_OPEN_ROM; // Pre-set to non-none

	LauncherNav_determineAction(NULL, "/path", NULL, &action);

	TEST_ASSERT_EQUAL(LAUNCHER_NAV_NONE, action.action);
}
...
void test_determineAction_null_action_does_not_crash(void) {
	...
	// Should not crash
	LauncherNav_determineAction(&entry, "/path", NULL, NULL);
}

void test_determineAction_collection_rom_sets_last_path(void) {
	Entry entry = {.path = "/mnt/SDCARD/Roms/GB/game.gb", .name = "game", .type = ENTRY_ROM};
	LauncherNavAction action;
	const char* collections_path = "/mnt/SDCARD/.launcher/Collections";
	const char* current_path = "/mnt/SDCARD/.launcher/Collections/Favorites";

	LauncherNav_determineAction(&entry, current_path, collections_path, &action);

	TEST_ASSERT_EQUAL(LAUNCHER_NAV_OPEN_ROM, action.action);
	// last_path should be collection path + filename
	TEST_ASSERT_EQUAL_STRING("/mnt/SDCARD/.launcher/Collections/Favorites/game.gb", action.last_path);
}
...
///////////////////////////////
// LauncherNav_shouldAutoLaunch Tests (using real temp dirs)
///////////////////////////////
...
void test_shouldAutoLaunch_with_cue_file(void) {
	...
	bool result = LauncherNav_shouldAutoLaunch(game_dir, launch_path, sizeof(launch_path));
...
void test_shouldAutoLaunch_empty_directory(void) {
	...
	char launch_path[512];
	bool result = LauncherNav_shouldAutoLaunch(temp_dir, launch_path, sizeof(launch_path));
...
void test_shouldAutoLaunch_null_inputs(void) {
	...
	TEST_ASSERT_FALSE(LauncherNav_shouldAutoLaunch(NULL, launch_path, sizeof(launch_path)));
	TEST_ASSERT_FALSE(LauncherNav_shouldAutoLaunch("/path", NULL, 512));
	TEST_ASSERT_FALSE(LauncherNav_shouldAutoLaunch("/path", launch_path, 0));
}

///////////////////////////////
// LauncherNav_buildPakCommand Tests
///////////////////////////////

void test_buildPakCommand_simple_path(void) {
	char cmd[512];

	LauncherNav_buildPakCommand("/mnt/SDCARD/Tools/Clock.pak", cmd, sizeof(cmd));
...
void test_buildPakCommand_path_with_spaces(void) {
	...
	LauncherNav_buildPakCommand("/mnt/SDCARD/Tools/My App.pak", cmd, sizeof(cmd));
...
void test_buildPakCommand_null_inputs(void) {
	...
	LauncherNav_buildPakCommand(NULL, cmd, sizeof(cmd));
	TEST_ASSERT_EQUAL_STRING("", cmd);

	cmd[0] = 'x';
	LauncherNav_buildPakCommand("/path", NULL, 512);
	// Should not crash

	LauncherNav_buildPakCommand("/path", cmd, 0);
	// cmd unchanged since size is 0
}

///////////////////////////////
// LauncherNavAction structure Tests
///////////////////////////////
...
///////////////////////////////
// LauncherNav_buildRomCommand Tests
///////////////////////////////

void test_buildRomCommand_basic(void) {
	char cmd[512], sd_path[512];

	LauncherNav_buildRomCommand("/mnt/SDCARD/Roms/GB/game.gb", "gambatte", "/mnt/SDCARD/Emus/GB.pak", false,
...
void test_buildRomCommand_preserves_rom_path(void) {
	...
	LauncherNav_buildRomCommand("/mnt/SDCARD/Roms/GBA/pokemon.gba", "gpsp", "/mnt/SDCARD/Emus/GBA.pak",
...
void test_buildRomCommand_handles_path_with_spaces(void) {
	...
	LauncherNav_buildRomCommand("/mnt/SDCARD/Roms/GB/My Game (USA).gb", "gambatte",
...
void test_buildRomCommand_null_inputs(void) {
	...
	// Null ROM path
	LauncherNav_buildRomCommand(NULL, "emu", "/emu/path", false, NULL, NULL, NULL, cmd, sizeof(cmd),
	                         sd_path, sizeof(sd_path));
	TEST_ASSERT_EQUAL_STRING("", cmd);

	// Null emu path
	cmd[0] = 'x';
	LauncherNav_buildRomCommand("/rom", "emu", NULL, false, NULL, NULL, NULL, cmd, sizeof(cmd), sd_path,
	                         sizeof(sd_path));
	TEST_ASSERT_EQUAL_STRING("", cmd);

	// Null cmd buffer - should not crash
	LauncherNav_buildRomCommand("/rom", "emu", "/emu", false, NULL, NULL, NULL, NULL, 512, sd_path,
	                         sizeof(sd_path));

	// Zero cmd size
	cmd[0] = 'x';
	LauncherNav_buildRomCommand("/rom", "emu", "/emu", false, NULL, NULL, NULL, cmd, 0, sd_path,
	                         sizeof(sd_path));
}
...
static LauncherContext* create_mock_context(LauncherCallbacks* callbacks) {
	static LauncherContext ctx;
...
void test_openPak_ctx_calls_save_last(void) {
	...
	LauncherNav_openPak(ctx, "/mnt/SDCARD/Tools/Clock.pak");
...
void test_openPak_ctx_null_callbacks_does_not_crash(void) {
	reset_mocks();
	LauncherContext* ctx = create_mock_context(NULL);

	LauncherNav_openPak(ctx, "/path");
...
void test_openDirectory_ctx_no_autolaunch_calls_open_directory(void) {
	...
	LauncherNav_openDirectory(ctx, "/mnt/SDCARD/Roms/GB", 0);
...
void test_openDirectory_ctx_null_context_does_not_crash(void) {
	...
	LauncherNav_openDirectory(NULL, "/path", 0);
...
void test_openDirectory_ctx_null_path_does_not_crash(void) {
	...
	LauncherNav_openDirectory(ctx, NULL, 0);
...
void test_closeDirectory_ctx_null_context_does_not_crash(void) {
	...
	LauncherNav_closeDirectory(NULL);
}
...
void test_closeDirectory_ctx_empty_stack_does_nothing(void) {
	...
	LauncherNav_closeDirectory(ctx);
}

void test_openRom_ctx_null_context_does_not_crash(void) {
	// Just verify it doesn't crash
	LauncherNav_openRom(NULL, "/path/rom.gb", NULL);
}
...
void test_openRom_ctx_null_path_does_not_crash(void) {
	...
	// Just verify it doesn't crash
	LauncherNav_openRom(ctx, NULL, NULL);
}

///////////////////////////////
// LauncherNav_openDirectory with autolaunch tests
///////////////////////////////
...
void test_openDirectory_autolaunch_with_cue_does_not_call_open_directory(void) {
	...
	LauncherNav_openDirectory(ctx, game_dir, 1);
...
///////////////////////////////
// LauncherNav_openEntry tests
///////////////////////////////

void test_openEntry_null_context_does_not_crash(void) {
	Entry entry = {.path = "/path", .name = "name", .type = ENTRY_ROM};
	// Should not crash
	LauncherNav_openEntry(NULL, &entry);
}
...
void test_openEntry_null_entry_does_not_crash(void) {
	...
	// Should not crash
	LauncherNav_openEntry(ctx, NULL);
}
...
void test_openEntry_pak_calls_openPak(void) {
	...
	LauncherNav_openEntry(ctx, &entry);
...
void test_determineAction_unknown_entry_type(void) {
	...
	LauncherNavAction action;
	action.action = LAUNCHER_NAV_OPEN_ROM; // Pre-set

	LauncherNav_determineAction(&entry, "/path", NULL, &action);
